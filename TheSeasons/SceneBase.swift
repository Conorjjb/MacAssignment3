...
class SceneBase: SKScene {
  ...
  var btnRight = SKSpriteNode(imageNamed: "arrow_right")
  ...
  func setUpFooter() {
    ...
    if getNextScene() != nil {
      btnRight.position = CGPoint(x: self.size.width/2 + 500, y: 38)
      btnRight.zPosition = 1
      addChild(btnRight)
    }
    ...
    if getPreviousScene() != nil {
      btnLeft.position = CGPoint(x: self.size.width/2 + 400, y: 28)
      btnLeft.zPosition = 1
      addChild(btnLeft)
    }
